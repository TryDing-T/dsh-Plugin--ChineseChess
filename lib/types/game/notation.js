import { clonePosition, parseCoordinate } from "./coordinates.js";
const CHINESE_NUMBERS = ['一', '二', '三', '四', '五', '六', '七', '八', '九'];
function pieceLabel(piece) {
    if (piece.type === 'general')
        return piece.side === 'red' ? '帅' : '将';
    if (piece.type === 'advisor')
        return piece.side === 'red' ? '仕' : '士';
    if (piece.type === 'elephant')
        return piece.side === 'red' ? '相' : '象';
    if (piece.type === 'horse')
        return '马';
    if (piece.type === 'rook')
        return '车';
...
function sourcePrefix(game, move) {
    ...
    if (index === 0)
        return '前';
    if (index === sameFilePieces.length - 1)
        return '后';
    return '中';
}
/**
 ...
 */
export function formatChineseMove(game, input) {
...
export function formatMoveRecord(record) {
    if (record.result === 'checkmate')
        return `${record.notation}将死`;
    if (record.givesCheck)
        return `${record.notation}将军`;
    return record.notation;
}
export function getPieceLabel(piece) {
    return pieceLabel(piece);
}
export function getPieceTypeLabel(type) {
