...
import { makeGameState, InvalidPositionError } from "./rules.js";
import { BOARD_HEIGHT, BOARD_SIZE, BOARD_WIDTH } from "./types.js";
const FEN_BY_TYPE = {
    general: 'k',
    advisor: 'a',
    elephant: 'b',
...
function parseFenPiece(code) {
    ...
    if (!type)
        throw new InvalidPositionError(`未知 FEN 棋子: ${code}`);
 ...
 */
export function serialize(game, options = {}) {
    if (options.format === 'fen')
        return toFen(game);
...
export function deserialize(serialized) {
    ...
    if (text.length === 0)
        throw new InvalidPositionError('不能反序列化空字符串');
    if (!text.startsWith('{'))
        return fromFen(text);
    let value;
    try {
...
function parseNonNegativeInt(value, label) {
    if (!/^\d+$/.test(value))
        throw new InvalidPositionError(`${label}不是非负整数`);
    return Number(value);
}
function parsePositiveInt(value, label) {
    const result = parseNonNegativeInt(value, label);
    if (result < 1)
        throw new InvalidPositionError(`${label}必须大于 0`);
    return result;
}
...
function parseMoveRecord(value, index) {
    ...
    if (typeof value.notation !== 'string')
        throw new InvalidPositionError(`历史第 ${index + 1} 步缺少棋谱`);
    if (typeof value.givesCheck !== 'boolean')
        throw new InvalidPositionError(`历史第 ${index + 1} 步将军标记错误`);
