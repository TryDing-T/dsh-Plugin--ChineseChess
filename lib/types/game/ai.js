 ...
 * 引擎不会失同步。
 */
import { generatePseudoMoves, isInCheckOnBoard, } from "./rules.js";
import { BOARD_SIZE, BOARD_WIDTH } from "./types.js";
...
function makeMoveInPlace(board, fromIndex, toIndex, context) {
    ...
    if (mover === null)
        throw new Error(`AI 内部错误: 起点 ${fromIndex} 无棋子`);
    board[toIndex] = mover;
    ...
    if (captured !== null)
        key ^= PIECE_HASH[toIndex * 14 + pieceCode(captured.side, captured.type)];
    context.key = key;
...
function unmakeMoveInPlace(board, fromIndex, toIndex, undo, context) {
    ...
    if (undo.captured !== null)
        key ^= PIECE_HASH[toIndex * 14 + pieceCode(undo.captured.side, undo.captured.type)];
    context.key = key;
}
const ROOK_PST = {
    rows: [
...
function legalMovesInPlace(board, side, includeCapturesOnly) {
    ...
    for (const move of pseudo) {
        if (includeCapturesOnly && move.captured === null)
            continue;
...
function outOfTime(context, ply) {
    if (context.timeMs < 0 || ply <= 0)
        return false;
    if ((context.nodes & 0x3ff) !== 0)
        return false;
...
function quiesce(board, side, alpha, beta, context, ply) {
    ...
    if (inCheck)
        captures.unshift(...moves.filter(move => move.captured === null));
...
function alphaBeta(board, side, depth, alpha, beta, context, ply) {
    ...
    if (hashEntry !== undefined && hashEntry.depth >= depth) {
        ...
        if (hashEntry.flag === 'exact')
            return storedScore;
        if (hashEntry.flag === 'lower' && storedScore >= beta)
            return storedScore;
        if (hashEntry.flag === 'upper' && storedScore <= alpha)
            return storedScore;
    }
    if (depth <= 0) {
    ...
    const ordered = [...moves].sort((left, right) => {
        ...
        if (leftKiller !== rightKiller)
            return leftKiller ? -1 : 1;
    ...
    for (const move of ordered) {
        ...
        if (alpha >= beta) {
            ...
            if (move.captured === null)
                ...
                rewardHistory(context, side, move, depth);
            flag = 'lower';
            break;
        }
    }
    ...
    if (bestMove !== null)
        rewardHistory(context, side, bestMove, depth);
    if (best <= startAlpha)
        flag = 'upper';
    else if (best >= beta)
        flag = 'lower';
 ...
 */
export function findBestMoves(game, options = {}) {
    ...
    const results = rootMoves.map(move => ({ move, score: 0 }));
    ...
    for (let depth = firstDepth; depth <= maxDepth; depth += 1) {
        ...
        if (context.aborted)
            break;
        if (hasTimeBudget && context.startTime + context.timeMs <= Date.now())
            break;
        ...
        for (const entry of results) {
            ...
            if (score > rootAlpha)
                rootAlpha = score;
            if (context.tt.size > TT_MAX_ENTRIES)
                context.tt.clear();
        }
